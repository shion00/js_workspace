...
/*------------------------------------------------------------------------------------------------
<해당월의 총 일수 구하기, 즉 마지막날 구하는 함수>
yy   : 구하고 싶은 년을 의미
mm : 구하고 싶은 월을 의미, 구하고자 하는 월에 -1 해야한다.
주의!) 자바스크립트에서 월은 0 부터 시작하므로
         만약 10월 구하려면 9를 입력해야 한다.
사용예시)
          getLastDate(d.getFullYear(),d.getMonth()); //코딩시 상수는 되도록 쓰지 않는다.
          31
          getLastDate(2020, 6); // 7월의 마지막일을 구하고자 하는 경우 -1해준다
          31
--------------------------------------------------------------------------------------------------*/ 
function getLastDate(yy,mm){//mm : 7월을 구하고자 한다면 6을 입력
    var d=new Date();//현재 날짜이므로, 조작이 가해져야 한다.
    d.setFullYear(yy);
    d.setMonth(mm+1);//일단 다음달로 넘어 간다..왜? 0을 대입하여 이전달의 마지막일을 구하기위해
